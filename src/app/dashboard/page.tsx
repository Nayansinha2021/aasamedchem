...
import { FileText, ShoppingCart, IndianRupee, ArrowRight, Clock, CheckCircle2, AlertCircle } from 'lucide-react';
  ...
  return (
    <DashboardLayout>
      <div className="space-y-8">
        {/* Header */}
        <div>
          <h2 className="text-3xl font-extrabold tracking-tight text-slate-800">Seller Dashboard</h2>
          <p className="text-slate-500 mt-1">Overview of your quotations, orders, and sales activity.</p>
        </div>
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="glass-panel glass-panel-hover rounded-2xl p-6 relative overflow-hidden bg-white">
                ...
                <p className="text-xs font-bold tracking-wider text-slate-400 uppercase">My Quotations</p>
                <h3 className="text-4xl font-extrabold text-slate-850 mt-4">{data?.stats.totalQuotations}</h3>
                <Link href="/quotations" className="inline-flex items-center text-xs font-semibold text-[#b48a5e] hover:text-[#9e7950] mt-6 group">
                  View Quotations <ArrowRight className="w-3.5 h-3.5 ml-1 transition-transform group-hover:translate-x-1" />
                </Link>
              </div>
              ...
              <div className="glass-panel glass-panel-hover rounded-2xl p-6 relative overflow-hidden bg-white">
                ...
                <p className="text-xs font-bold tracking-wider text-slate-400 uppercase">My Orders</p>
                <h3 className="text-4xl font-extrabold text-slate-850 mt-4">{data?.stats.totalOrders}</h3>
                <Link href="/orders" className="inline-flex items-center text-xs font-semibold text-[#a67c52] hover:text-[#8f6943] mt-6 group">
                  View Orders <ArrowRight className="w-3.5 h-3.5 ml-1 transition-transform group-hover:translate-x-1" />
                </Link>
              </div>
              ...
              <div className="glass-panel glass-panel-hover rounded-2xl p-6 relative overflow-hidden bg-white">
                ...
                <p className="text-xs font-bold tracking-wider text-slate-400 uppercase">Total Spending</p>
                <h3 className="text-4xl font-extrabold text-slate-850 mt-4 truncate">
                  ₹{Number(data?.stats.totalSpending).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 5 })}
                </h3>
                <p className="text-xs text-slate-500 mt-6">Calculated from approved orders</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="glass-panel rounded-2xl p-6 bg-white border border-[#eae3d2]">
                <div className="flex justify-between items-center mb-6">
                  <h4 className="text-lg font-bold text-slate-800">Recent Quotations</h4>
                  <Link href="/quotations" className="text-xs font-semibold text-[#b48a5e] hover:text-[#9e7950]">View All</Link>
                </div>
              ...
              <div className="glass-panel rounded-2xl p-6 bg-white border border-[#eae3d2]">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <Link href="/orders" className="text-xs font-semibold text-[#a67c52] hover:text-[#8f6943]">View All</Link>
                </div>
