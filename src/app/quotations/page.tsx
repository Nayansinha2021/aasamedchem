...
import { FileText, ShoppingCart, IndianRupee, Clock, CheckCircle2, AlertCircle, RefreshCw } from 'lucide-react';
...
export default function SellerQuotationsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'PENDING':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100/80 text-amber-800 border border-amber-200"><Clock className="w-3.5 h-3.5 mr-1" /> Pending Approval</span>;
      case 'APPROVED':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-100/80 text-emerald-800 border border-emerald-200"><CheckCircle2 className="w-3.5 h-3.5 mr-1" /> Approved (Order Created)</span>;
      case 'REJECTED':
  ...
  return (
    <DashboardLayout>
      <div className="space-y-8">
        {/* Header */}
        <div>
          <h2 className="text-3xl font-extrabold text-slate-800">My Quotations</h2>
          <p className="text-slate-505 mt-1">Review your generated quotations or convert pending ones to orders.</p>
        </div>

        {actionError && (
          <div className="p-4 bg-rose-500/5 border border-rose-500/10 rounded-2xl text-rose-700 text-sm">
            {actionError}
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="py-16 text-center text-slate-500 bg-[#eae3d2]/10 rounded-2xl border border-[#eae3d2]/40">
            No quotations found. Go to products to generate one.
          </div>
        ) : (
          <div className="space-y-6">
            {quotations.map((quote) => (
              <div key={quote.id} className="glass-panel rounded-2xl p-6 border border-[#eae3d2]/80 space-y-4 bg-white">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[#eae3d2]/60">
                  <div>
                    <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400">Quotation ID</span>
                  ...
                  <div className="flex items-center space-x-6">
                    <div>
                      ...
                      <span className="text-xs text-slate-500 font-semibold">{new Date(quote.createdAt).toLocaleString()}</span>
                    </div>
                ...
                <div className="space-y-2">
                  <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400 block mb-2">Requested Items</span>
                ...
                <div className="pt-4 border-t border-[#eae3d2]/60 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  {quote.status === 'PENDING' && (
                    <button
                      ...
                      className="px-5 py-2.5 bg-[#b48a5e] hover:bg-[#9e7950] disabled:opacity-50 text-white text-xs font-semibold rounded-xl flex items-center space-x-2 transition shadow-md shadow-[#b48a5e]/15"
                    >
                      {actionId === quote.id ? (
